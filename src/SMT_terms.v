...
Module Typ.
  ...
  Section Interp.
    ...
    Section Interp_Equality.
      ...
      Fixpoint list_eqb (l1 : list int) (l2 : list int) :=
        match (l1,l2) with
        | (nil,nil) => true
        | ((a1::r1),(a2::r2)) => (a1==a2) && (list_eqb r1 r2)
        | _ => false
        end.
      ...
      Lemma list_eqb_refl : forall l, list_eqb l l.
      Proof.
        induction l;intros.
        unfold list_eqb; eauto.
        unfold list_eqb; apply andb_true_iff; split. 
        apply Int63Properties.eqb_spec; trivial.
        apply IHl.
      Qed.

      Lemma list_eqb_spec : forall l1 l2, list_eqb l1 l2 <-> l1 = l2.
      Proof.
        induction l1;induction l2;intros;split;intros.
        trivial.
        apply list_eqb_refl.
        inversion H.
        rewrite H; apply list_eqb_refl.
        inversion H.
        inversion H.
        inversion H; apply andb_true_iff in H1; destruct H1; apply IHl1 in H1; rewrite H1; apply Int63Properties.eqb_spec in H0; rewrite H0; trivial.
        rewrite H; apply list_eqb_refl.
      Qed.
  ...
  Section Typing_Interp.
    ...
    Section Typ_Aux.
      ...
      Proof.
        ...
        (* Unary operators *)
        destruct op; simpl; try (case (Typ.eqb (get_type h) Typ.Tpositive); [left; exists Typ.Tpositive|right; intro; rewrite andb_false_r]; reflexivity); try (case (Typ.eqb (get_type h) Typ.Tpositive); [left; exists Typ.TZ|right; intro; rewrite andb_false_r]; reflexivity);try (case (Typ.eqb (get_type h) Typ.TZ); [left; exists Typ.TZ|right; intro; rewrite andb_false_r]; reflexivity);try (case (Typ.eqb (get_type h) Typ.Tint); [left; exists Typ.Tint|right; intro; rewrite andb_false_r]; reflexivity);case (Typ.eqb (get_type h) Typ.Tint); [left; exists Typ.Tbool|right; intro; rewrite andb_false_r]; reflexivity.
        (* Binary operators *)
    ...
    Section Interp_Aux.
     ...
     Fixpoint compute_interp ty acc (l:list bval) :=
      ...
      Lemma compute_interp_spec : forall ty l acc,
        match compute_interp ty acc l with
          | Some l' => forall i j, In2 i j l' <-> (In2 i j acc \/ (In j acc /\ exists a, In a l /\ a = Bval ty i) \/ (exists a b, In2 b a l /\ a = Bval ty i /\ b = Bval ty j))
          | None => exists a, In a l /\ let (ta,_) := a in ta <> ty
        end.
      Proof.
        intro ty; induction l as [ |a q IHq]; simpl.
        intros acc i; split.
        ...
        elim H.
        inversion H.
        intro acc; case_eq (a); intros ta va Heq; rewrite Typ.neq_cast; case_eq (Typ.eqb ta ty).
        change (Typ.eqb ta ty = true) with (is_true (Typ.eqb ta ty)); rewrite Typ.eqb_spec; intro; subst ta; rewrite Typ.cast_refl; generalize (IHq (va :: acc)); clear IHq; case (compute_interp ty (va :: acc) q).
        ...
        inversion H; clear H.
        subst va; right; right; exists b; exists a; repeat split; auto; subst a; constructor 1; auto.
        ...
        left; constructor 2; auto.
        subst b; generalize (Bval_inj2 _ _ _ H2); intro; subst va; left; constructor; auto.
        right; left; split.
        constructor 2; auto.
        exists b; auto.
        inversion H; clear H.
        subst c l0; generalize (Bval_inj2 _ _ _ H0); intro; subst va; right; left; split.
        constructor 1; auto.
        exists b; auto.
      ...
      Lemma compute_interp_spec_rev : forall ty l,
        match compute_interp ty nil l with
          | Some l' => forall i j, In2 j i (rev l') <-> (exists a b, In2 b a l /\ a = Bval ty i /\ b = Bval ty j)
          | None => exists a, In a l /\ let (ta,_) := a in ta <> ty
        end.
      Proof.
...
Proof.
 ...
 Qed.
      ...
      Lemma forallb_array_false : forall (A : Type) (f : A -> bool) (a : array A), (0 < length a) -> PArray.forallb f a = false -> (exists i : int, i < length a /\ (f (a.[i]) = false)).
      Proof.
        intros A f a H H0.
        unfold forallb in H0.
        assert ((0 == length a) = false).
        rewrite Int63Properties.eqb_false_spec.
        apply not_eq_sym.
        rewrite not_0_ltb.
        apply H.
        rewrite H1 in H0.
        ...
        inversion_clear H0.
        destruct H2 as (H0,H2); destruct H2 as (H2,H3).
        rewrite <- ltb_leb_sub1 in H2.
        exists x;split;[apply H2|apply H3].
        rewrite Int63Properties.eqb_false_spec in H1;apply not_eq_sym;apply H1.
      Qed.
...
SearchAbout forallb.
      ...
      Proof.
        ...
        assert (forall l, In x (to_list ha) -> compute_interp Typ.Tint l (to_list (mapi (fun _ x1 : int => interp_hatom x1) ha)) = None).
        induction (to_list ha);intros H01 H00.
        inversion H00.
        inversion H00.
        subst a;unfold compute_interp;simpl.
        assert (Typ.cast (get_type x) Typ.Tint = Typ.NoCast _ _) as H.
        (* detruit H0 -> absurd si x >= length ha*)
